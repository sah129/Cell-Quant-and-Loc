...
```{r}
get_boundaries <- function(cmac)
{
  cells <- cmac > otsu(cmac, levels = 65536)
  #cells<- opening(cells, kern = makeBrush(7, shape = 'disc'))
 # cells <-  opening(cells, kern = makeBrush(7, shape = 'Gaussian'))
  cells <- bwlabel(cells)
  return(cells)
}
```
...
```{r}
cells<-get_boundaries(normalize(channels$cmac))
fc<-computeFeatures.shape(cells)
sel <- which(fc[,"s.area"]<1000)
cells<-rmObjects(cells,sel)
#fc<-computeFeatures(cells, ref = channels$gfp, xname = "cell")
...
```{r}
c<-normalize(channels$cmac + channels$gfp + channels$ip)
c<- c > otsu(c)
display(c)
co <- opening(c, kern = makeBrush(7, shape = 'Gaussian'))
display(co)
