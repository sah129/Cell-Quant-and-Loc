...
detect_membranes <-function(img)
{
  ...
  FMS <- computeFeatures(membranes, ref = img[,,gfp_channel], xname = "membrane")
  ...
  list(removed = res$removed, membranes = res$membranes, FMS = FMS)
...
```{r}
dff <- data.frame(CellID = 0,
                    vacuoles = I(list(c(1,1))), 
                    cell_area = 100, 
                    vac_area = 200, PM_vac_ratio = .5, cell_mpi = 1, vac_mpi = 2)
```
...
```{r}
dfn <- data.frame(matrix(NA, nrow = length(table(c$membranes)), ncol = 7))
names(dfn) <- c('CellID', 'vacuoles', 'cell_area', 'vac_area', 'PM_vac_ratio', 'cell_mpi', 'vac_mpi')
...
FMB = computeFeatures.moment(c$membranes, ref = imggray[,,gfp_channel], xnames = "membrane")
FMV = computeFeatures.moment(vacs$vacuoles, ref = imggray[,,gfp_channel], xnames = "membrane")
...
```{r}
exclude_and_bind <- function(mems, vacs,df)
{
...
```{r}
newdf <- exclude_and_bind(c, vacs, dfn)
```
