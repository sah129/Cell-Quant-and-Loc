
```{r setup, include=FALSE}
source('src/functions.R')
source('src/Main.R')
datasetpath <- "Datasets/Figure S1"
```

"Datasets/Figure 2C Bul Data/Bul1-del"
"Datasets/Figure 2C Bul Data/Bul1-del+Bul2-del"
"Datasets/Figure 1C/9ArrD 316-9ArrD Aly1-9ArrD Aly2-9ArrD Csr2-9ArrD Ecm21"

```{r}
res <- pipeline(datasetpath, testing=FALSE, gui=FALSE, progress=NULL, interactive = FALSE)
```

```{r}
#groups = list("9ArrD 316", "9ArrD Aiy1", "9ArrD Aly2", "9ArrD Csr2", "9ArrD Ecm21", "9ArrD Ldb19", "9ArrD Rod1", "9ArrD Rog3", "9ArrD Ylr392c", "9ArrDYgr068c", "WT 316")
#groups = list("9Arr-del","aly1-del") 
#groups = list("Bul1-del")
#groups =  list("Aly1delAly2del", "Apl1del", "Apl3del", "BY4741", "End3del", "Vrp1del")
#groups = list("Bul1-del+Bul2-del","Bul2-del")
#groups = list("aly1-del_aly2-del", "aly2-del", "BY4741")
#groups = list("9ArrD 316", "9ArrD Aiy1", "9ArrD Aly2", "9ArrD Csr2", "9ArrD Ecm21", "9ArrD Ldb19", "9ArrD Rod1", "9ArrD Rog3", "9ArrD Ylr392c", "9ArrDYgr068c")

#groups = list("Aly1delAly2del", "Apl1del", "Apl3del", "BY4741", "Cnb1del", "End3del", "Vrp1del")
#groups = list("DMSO_50uMGPI_120min", "LatA_50uMGPI_120min", "DMSO_NoGPI", "LatA_NoGPI")
#groups = list("Time0", "Time60", "Time120")
groups = list("9Arr-del_415Git1", "aly1-del_415Git1", "aly1-del_aly2-del_415Git1", "aly2-del_415Git1", "Bul1-del_415Git1", "Bul1-del+Bul2-del_415Git1", "Bul2-del_415Git1", "BY4741(2)_415Git1", "BY4741_415Git1")
sort_data(res, groups, "Figure S1")
```

```{r}
r <- res[[2]]
```


```{r}
get_display_img(df = r$df,
                membranes = r$membranes, 
                col_membranes = 'white', 
                vacuoles = r$vacuoles, 
                col_vacuoles ='yellow', 
                removed = r$removed,
                closed_vacuoles = TRUE, 
                img = r$channels$gfp, 
                showRemoved = FALSE, 
                showMemLabels = TRUE, 
                showVacLabels = TRUE)
```





##############################################################3



```{r}
get_image_names <- function(res)
{
  image_names = vector(mode = "list",length = length(res))
  for(i in 1:length(res))
  {
    image_names[i] <- res[[i]]$filename
  }
  return(image_names)
}
```




```{r}
get_max_cells <- function(res)
{
  max = -1
  for(i in 1:length(res))
  {
    if( max < nrow(res[[i]]$df["cell_mpi"]))
      max = nrow(res[[i]]$df["cell_mpi"])
  }
  return(max)
}

```


```{r}
max_cells <- get_max_cells(res)

```

```{r}
create_dummy_row <- function(row_text, m)
{
  df <- data.frame(Image = row_text)
  ids <- vector("list", m)
  for(j in 1:m)
  {
   x <- data.frame(NA)
    names(x) <- j
    ids[[j]]<- x
  }
  df <- cbind(df, ids)
  df
}
```



